...
async function harvest() {
    ...
    tokenHarvestThreshold = getWeiTokens(farmDecimal, tokenHarvestThreshold / poolIDS.length);
    tokenSellThreshold = getWeiTokens(farmDecimal, sellThreshold / price[2]);
    console.log(tokenHarvestThreshold, tokenSellThreshold);
    for (var i = 0; i < poolIDS.length; i++) {
        try {
            var poolId = poolIDS[i];
            var pendingBone = await stakingContract.methods.pendingBone(poolId, account).call();
            console.log("pendingBone - " + pendingBone);
    ...
    try {
        ...
        console.log("Balance " + balance, getWeiTokens(farmDecimal, balance));
        if (balance > tokenSellThreshold) {
            ...
            price = await getPrice(balance);
            console.log(price);
            var buyResult = await sendBuy(balance, price[1]);
        }
    } catch (ex) {
        console.log(ex);
    }
...
async function sendHarvest(poolId) {
    var data = stakingContract.methods.deposit(
...
async function getPrice(inToken) {
    if (inToken == 0) return [0, 0, 0];
    var amount = await qsRouter.methods.getAmountsOut(inToken, [farmTokenAdd, maticAddress]).call();
    if (!amount) {
        return false;
    }
    ...
    console.log("Price " + price + " bone = 1 matic");
    console.log("Price " + invPrice + " matic = 1 bone");
...
async function sendBuy(inAmount, outAmount) {
