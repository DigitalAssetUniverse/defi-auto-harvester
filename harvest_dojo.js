...
const fs = require('fs');
let rawdata = fs.readFileSync('wallet_dojo.json');
...
async function harvest() {
    ...
    var price = await getPrice(10, maticAddress);
    tokenHarvestThreshold = harvestThreshold / (price[2]/10);
    tokenHarvestThreshold = getWeiTokens(farmDecimal, tokenHarvestThreshold);
    tokenSellThreshold = getWeiTokens(farmDecimal, sellThreshold /(price[2]/10));
    console.log(getReverseWeiTokens(tokenHarvestThreshold,farmDecimal).toFixed(fDec),
	getReverseWeiTokens(tokenSellThreshold,farmDecimal)	);
    for (var i = 0; i < poolIDS.length; i++) {
        try {
            var poolId = poolIDS[i];
            var pendingBone = await stakingContract.methods.pendingDojo(poolId, account).call();
            console.log("pending - " + getReverseWeiTokens(pendingBone,farmDecimal).toFixed(fDec));
    ...
    try {
        ...
        if (balance > tokenSellThreshold) {
            ...
            console.log(price);
            var buyResult = await sendBuy(balance, parseInt(price[1]), wallet.toContract);
        }
    } catch (ex) {
        console.log(ex);
    }
...
async function getPrice(inToken, toToken) {
    ...
    console.log("Price " + price + " Token = 1 matic");
    console.log("Price " + invPrice + " matic = 1 Token");
    var outAmount = outOgAmount;
    outAmount = (outAmount - (outAmount * wallet.slippage / 100)); //*Math.pow(10,contractPairDecimal);
    return [inAmount, outAmount, outOgAmount];
}

function formatTokens(balance, decimals) {
    if (!balance || balance == 0) return 0;
    return parseInt(balance * Math.pow(10, decimals));
}
...
async function sendBuy(inAmount, outAmount, toContract) {
    ...
    var data = qsRouter.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
